...
class MainDetailPageViewModel{
    ...
    //         }
    //
    //         let newsListArray = [newsList, newsList1] // 두 뉴스 리스트를 하나의 배열로 만듭니다.
    //
    //         input.newsTrigger
    //             .flatMapLatest { _ in
    ////                 self.newsListRelay.accept([self.newsList])
    ////                 self.newsListRelay.accept([self.newsList1])
    //                 self.newsListRelay.accept(newsListArray) // 뉴스 리스트 배열을 전달
